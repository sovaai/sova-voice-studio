...
declare module Immutable {
  ...
  export interface Set<T> extends Collection.Set<T> {
     ...
     */
    flatMap<M>(mapper: (value: T, key: T, iter: this) => Iterable<M>, context?: any): Set<M>;
    ...
    filter(predicate: (value: T, key: T, iter: this) => any, context?: any): this;
  }
  ...
  export interface OrderedSet<T> extends Set<T> {
     ...
     *     // OrderedSet [10, 20]
     */
    map<M>(mapper: (value: T, key: T, iter: this) => M, context?: any): OrderedSet<M>;
     ...
     * Similar to `set.map(...).flatten(true)`.
     */
    flatMap<M>(mapper: (value: T, key: T, iter: this) => Iterable<M>, context?: any): OrderedSet<M>;
  ...
  export module Seq {
    ...
    export interface Set<T> extends Seq<T, T>, Collection.Set<T> {
       ...
       * same value at every step.
       */
      map<M>(mapper: (value: T, key: T, iter: this) => M, context?: any): Seq.Set<M>;
       ...
       * Similar to `seq.map(...).flatten(true)`.
       */
      flatMap<M>(mapper: (value: T, key: T, iter: this) => Iterable<M>, context?: any): Seq.Set<M>;
  ...
  export interface Seq<K, V> extends Collection<K, V> {
     ...
     */
    map<M>(mapper: (value: V, key: K, iter: this) => M, context?: any): Seq<M, M>;
     ...
     * Note: Used only for sets.
     */
    flatMap<M>(mapper: (value: V, key: K, iter: this) => Iterable<M>, context?: any): Seq<M, M>;
  ...
  export module Collection {
    ...
    export interface Set<T> extends Collection<T, T> {
       ...
       * same value at every step.
       */
      map<M>(mapper: (value: T, key: T, iter: this) => M, context?: any): Collection.Set<M>;
  ...
  export function remove<TProps, C extends Record<TProps>, K extends keyof TProps>(
    collection: C,
    key: K
  ): C;
  ...
  export function update<TProps, C extends Record<TProps>, K extends keyof TProps>(
    record: C,
    key: K,
    updater: (value: TProps[K]) => TProps[K]
  ): C;
  export function update<TProps, C extends Record<TProps>, K extends keyof TProps, NSV>(
    record: C,
    key: K,
    notSetValue: NSV,
  ...
  export function update<V, C extends { [key: string]: V }, K extends keyof C, NSV>(
    collection: C,
    key: K,
    notSetValue: NSV,
    updater: (value: V | NSV) => V
